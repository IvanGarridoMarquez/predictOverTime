...
v=args.verbose
tictac=args.time
resDirSV=args.output+blog+"/"
plotsDir="../plots3/"+blog+"/"


if not os.path.exists(resDirSV):
    os.makedirs(resDirSV)
...
#vectorize the pre_train_set in feature space
    yar=flog.yearArray(pre_train_set_parse)
    featC,featTI,tiMat,cntMat,tiMatWa=ct.getWeighBagOfWordsYr(ct.loadStopwords("stopwords.french.list"),pre_train_set_parse,wa,yar)
    if v:
...
#train multi-label classifier=predictor
    #print tiMat.getrow(0)
    #print "--###---"
    #print tiMatWa.getrow(0)
    #exit()
    ...
    predictorWa=ct.trainPredictor(tiMatWa,labels_trn)
    if v:
...
#get all posterior examples to curr_yr=test_set
    for year_tst in range(curr_year+1,last_year+1):      
        ...
        test_set=curBlog.getPostsBetweeDates(str(year_tst)+"-01-01",str(year_tst)+"-12-31")
        if v:
            print "test set range:["+str(year_tst)+"-"+str(year_tst)+"]-test_total:"+str(len(test_set))
        test_set_parse=flog.microCorpus(test_set)
    #vectorize the test_set in feature space
        yrr=flog.yearArray(test_set_parse)
        tiMat_tst,cntMat_tst=ct.transformToFeatYr(featC,featTI,test_set_parse,yrr)
    ...
    #evaluate
        acc_score,prec_score,rec_score,f1_score=ct.evaluateMultilabelPrediction(lbls_predicted,true_labels_tst)
        acc_scoreWa,prec_scoreWa,rec_scoreWa,f1_scoreWa=ct.evaluateMultilabelPrediction(lbls_predictedWa,true_labels_tst)
    ...
    #(confusion matrix part removed the multilabel indicator is not supported I will have to make this on my own)
        output = open(resDirSV+blog+'_'+str(curr_year)+'_'+str(year_tst)+'_predictions.pkl', 'wb')
        print list(handLbl.classes_)        
        cPickle.dump([true_labels_tst,lbls_predicted,lbls_predictedWa], output)
        output.close()
        
#        outputCnfMt = open(resDirSV+blog+'_'+str(curr_year)+'_'+str(year_tst)+'_confusion.pkl', 'wb') 
#        cPickle.dump([cnfMTX,cnfMTXWa], outputCnfMt)
#        outputCnfMt.close()
        
        if v:
            print "scores[accuracy,precision,recall,f1-measure]:"
            print "s--["+str(curr_year)+"|"+str(year_tst)+"]--"+str(acc_score)+","+str(prec_score)+","+str(rec_score)+","+str(f1_score)
            print "scoresWa[accuracy,precision,recall,f1-measure]:"
            print "sw--["+str(curr_year)+"|"+str(year_tst)+"]--"+str(acc_scoreWa)+","+str(prec_scoreWa)+","+str(rec_scoreWa)+","+str(f1_scoreWa)
...
output = open(resDirSV+blog+'_ACC_predTime.pkl', 'wb')
cPickle.dump(acc_curve, output)
output = open(resDirSV+blog+'_PRE_predTime.pkl', 'wb')
cPickle.dump(prec_curve, output)
output = open(resDirSV+blog+'_REC_predTime.pkl', 'wb')
cPickle.dump(rec_curve, output)
output = open(resDirSV+blog+'_F1M_predTime.pkl', 'wb')
cPickle.dump(f1_curve, output)

output = open(resDirSV+blog+'_ACCw_predTime.pkl', 'wb')
cPickle.dump(acc_curveWA, output)
output = open(resDirSV+blog+'_PREw_predTime.pkl', 'wb')
cPickle.dump(prec_curveWA, output)
output = open(resDirSV+blog+'_RECw_predTime.pkl', 'wb')
cPickle.dump(rec_curveWA, output)
output = open(resDirSV+blog+'_F1Mw_predTime.pkl', 'wb')
...
plOT.plotBehaviorN(xlb,dat_crv,lb_crv,legends,plotsDir+blog+"_acc_svc_ovrtm.png")
plOT.plotBehaviorN(xlbWA,dat_crvWA,lb_crv,legends,plotsDir+blog+"_accWA_svc_ovrtm.png")
...
plOT.plotBehaviorN(xlb,dat_crv,lb_crv,legends,plotsDir+blog+"_prec_svc_ovrtm.png")
plOT.plotBehaviorN(xlbWA,dat_crvWA,lb_crv,legends,plotsDir+blog+"_precWA_svc_ovrtm.png")
...
plOT.plotBehaviorN(xlb,dat_crv,lb_crv,legends,plotsDir+blog+"_rec_svc_ovrtm.png")
plOT.plotBehaviorN(xlbWA,dat_crvWA,lb_crv,legends,plotsDir+blog+"_recWA_svc_ovrtm.png")
...
plOT.plotBehaviorN(xlb,dat_crv,lb_crv,legends,plotsDir+blog+"_f1_svc_ovrtm.png")
plOT.plotBehaviorN(xlbWA,dat_crvWA,lb_crv,legends,plotsDir+blog+"_f1WA_svc_ovrtm.png")
